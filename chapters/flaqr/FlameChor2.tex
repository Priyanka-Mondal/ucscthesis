...
allowing hosts (i.e. locations) to communicate without the sender host waiting 
for the receiver host to receive the message. 
%
This means, the sender should be able go forward in its local program and the its message will 
reside in the receiver's message queue. 
%
...
%
To understand how the asynchronous communication works, let us go through an example in Figure \ref{fig:ServerClientAsync}. 
%
The example is written using the new extended version of \HasChor\ syntax, that supports asynchronous 
send and receives. 
%
In this example, the server sends an integer value \hs{a} to the client (line \ref{ex:asyncSend}). And, without waiting for the client to receive the value, it proceeds in its local program (line \ref{ex:asyncCont}). 
%
The client receives the message from its queue using the \hs{wait} keyword when its ready (line \ref{ex:asyncWait}). 
%
Figure \ref{fig:Async} shows the programs running at the server and client corresponding to the 
choroegraphy in Figure \ref{fig:ServerClientAsync}.
%
...
\hs{ChoreoSig m (a @ l')}.  \hs{ChoreoSig m (Async a @ l')}  specifies that the result of this operation is an \emph{asynchronous} value of type \hs{a} located at  \hs{l'}. The corresponding definition of \hs{locally} and \hs{(}$\sim$\hs{>)} are shown in Figure \ref{fig:haschorAsyncChoreoSig}. 
...
asyncServerClient = do
  ...
  c <- (server, a) |$\sim$|> client |\label{ex:asyncSend}|
  client `locally` \un -> do 
     q <- wait (un abc)    |\label{ex:asyncWait}|
  server `locally` (\_ -> putStrLn "Do other stuff!" |\label{ex:asyncCont}|
 return q
 \end{minted}
\end{minipage}
\hfill
...
(|$\sim$|>) :: (Show a, Read a, KnownSymbol l, KnownSymbol l')
...
%
The \hs{IFC} type class has three parameters: the monad \hs{m}, the effect we want to control \hs{e} (in this dissertation we will only consider IO effects), and an instance of the \hs{Labeled} type class that enforces noninterference on pure computation, \hs{n}  (see Figure. \ref{fig:flameCore}). 
%The \hs{e} parameter represents an effect we want to control . The \hs{n} is an instance of the \hs{Labeled} type class that enforce noninterference on pure computation.
...
\label{fig:flameCore}
\end{figure*}
The \hs{IFC} class has three methods. 
%
The \hs{protect} method in figure \ref{fig:flameCore} has type \hs{ m e n pc l a}. 
%
The simplest way to interpret this type is as follows: it associates a label 
\hs{l} with the result of a computation of type \hs{a}, 
%
and a program counter label \hs{pc} which bounds the confidentiality and integrity of side-effects \hs{e}. 
%
The \hs{protect} function realizes the \ruleref{Sealed} rule, i.e. given any term, \hs{protect} labels the term and lifts it into \hs{IFC} type where \hs{pc} $\sqsubseteq$ \hs{l}. 
%

The \hs{use} function realizes the \ruleref{Bind} rule. 
%
...
that \hs{l} $\sqsubseteq$ \hs{l'}, \hs{pc} $\sqsubseteq$ $pc$' and 
\hs{l} $\sqsubseteq$ $pc$'.
% (\hs{pc} $\sqcup$ \hs{l}) $\sqsubseteq$ $pc$'. 
The \hs{runIFC} function executes a protected computation, 
which results in a labeled value of type \hs{(n l a)} in the desired effect \hs{e}.
...
\end{minipage}
\caption{Flame principals (newly added availability principals are highlighted in \tblue{blue})}
...
\end{minted}
\caption{Some useful type synonyms for Flame principals (the new additions are shown in \tblue{blue})}
...
\end{minted}
\caption{A GADT defining singleton types for each member of \hs{KPrin} and functions for
notational convenience  (the new additions are shown in \tblue{blue}).}
\label{fig:sprin}
\end{figure*}
...
%
The "Top" and "Bot" are special principals correspond to the principals $\top$ and $\bot$.
%
...
%
The newly added authority projection for availability is \tblue{"Avail"} 
(shown in \tblue{blue} in Figure  \ref{fig:flamePrincipalsa}).
%
...
Figure \ref{fig:sprin} defines \hs{SPrin}, a generalized algebraic data type (GADT) for
principal singletons. In \FLAME\ the SPrin operations are prepended with an
asterisk *.
%
Each \hs{KPrin p} is uniquely represented by \hs{SPrin p}.
%
Therefore, if we can link a run-time principal with the corresponding singleton, we
would have a mechanism for reasoning about a value in type-level computations. 
%
In order to associate a \hs{Prin} value to \hs{SPrin}, \FLAME\ uses a 
data type (see Figure \ref{fig:flameEx}) for existential quantification during compile time.
...
promote p =
  case p of
    ...
    |\tblue{(Avail p) -> case promote p of Ex p' -> Ex (SAvail p')}|

\end{minted}
\caption{Promoting run-time principals to the type level. An \hs{SPrin} witness is created for each
run-time principal. Client code may extract this witness by pattern matching on the return value.}
\label{fig:flameEx}
\end{figure}
...
Hence, we define the \hs{LabeledSig} data type, in the similar style to \hs{ChoreoSig}. 
%
This \hs{LabeledSig} data type substitutes the \hs{IFC} class in the original version of \Flame. 
%
\hs{LabeledSig} has three constructors: \hs{Protect}, \hs{Use} and \hs{Restrict}. 
%
The \hs{Protect} constructor mirrors the \hs{protect} function of \hs{IFC} class in \Flame, i.e. it realizes the \ruleref{Sealed} operation on \FLAQR.  
%
The \hs{protect} constructor simply protects a value of type \hs{a} with the security label \hs{l} inside the labeled computation in the monad \hs{m}.
%
The \hs{Use} constructor mirrors the \hs{use} function of the \hs{IFC} class, i.e. it realizes the \ruleref{Bind} rule of \FLAQR. 
%
To put it in the simplest way, this constructor allows the use of a labeled value in a computation. 
%
It takes two arguments. The first argument \hs{l'!b} represents a value of type \hs{b} labeled with security label \hs{l'}. 
%
...
%
It can be thought of as an interface between the two libraries.
%
...
at a certain program counter level. 
...
%
We have not extended the \hs{cond} (conditions) syntax from \HasChor, 
although it can be an interesting future work.

\subsubsection{Local computation.}
The \hs{sLocally} function (similar to \hs{locally} in \hs{HasChor}) performs local computation at a specified location that respects certain security properties and information flow constraints. 
%
The function's first set of inputs (as shown in Figure \ref{fig:slocally}) are polymorphic type variable for the program counter \hs{pc}, 
location \hs{loc} (which is a known string), location's program counter \hs{loc_pc},
and the security label \hs{l}. 
%
The second argument to \hs{sLocally} is a function\hs{(Unwrap loc -> Labeled m loc_pc (l!a))}: 
the unwrap function allows the local computation to unwrap values located at \hs{loc}, 
and the return type is a protected value of type \hs{a}. 
%
The return type of \hs{sLocally} is a labeled computation within the \hs{Choreo} monad, 
labeled with \hs{pc}, producing a value of type \hs{(l!a)} at location \hs{loc}.
%
...
from \HasChor, and shown in Figure \ref{fig:scomm}. 
%
The first argument to it is a quadruple consisting of sender's location \hs{loc}, a clearance \hs{pc}, security label \hs{l}, and a value located at the sender \hs{(l ! a)@loc}.
%
The second argument is the receiver's location \hs{loc'}, 
and the output is an \hs{Async} value labeled with \hs{l} and the \hs{pc}, 
inside the \hs{Labeled} monad. 
%
The implementation uses the \hs{restrict} function as it works as a wrapper for safe interaction between communication operation \hs{(}$\sim$\hs{>)} in \HasChor\ and IFC in \Flame. 
%

The helper functions \hs{labelInA} and \hs{joinLoc} functions are shown in \ref{fig:helpers}. 
The \hs{labelInA} function converts a type \hs{l!(a @ loc)} to a type \hs{(l!a)@loc}. 
The \hs{joinLoc} function converts a type  \hs{(l!(l'!a)) @ loc} to \hs{(l''!a) @ loc},
if \hs{l}$\sqsubseteq$\hs{l''} and \hs{l'}$\sqsubseteq$\hs{l''}.
...
\begin{minted}[escapeinside=||]{haskell}
joinLoc :: forall l l' l'' loc a. (l |$\sqsubseteq$| l'', l' |$\sqsubseteq$| l'') => 
                        (l!(l'!a)) @ loc -> (l''!a) @ loc
