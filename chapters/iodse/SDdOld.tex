...
%\end{multicols}
  \end{mdframed}
  \vspace{-0.2em}
	\caption{{\sf SD}$_d$/\LSDd: from {\sf $\Gamma$=(\texttt{KeyGen}, \Setup, \Search)} to DSE (de-amortized version).}
	\label{fig:SSEtoDSE1}
\end{figure}
...
\begin{algorithmic}[1]
		\item[Client $\leftrightarrow$ Server:]
			...
			%\State Server initializes $L_{i-1}.{\sf OLDER}$ as an empty map of size $2^{i-1}
...
\caption{\SDd/\LSDd[$\Gamma$] framework. (a) The first element inserted to $\NEW_0$ (Rule 1). $\NEW_0$ becomes full and it is moved to $\OLDEST_0$ (Rule 2). Similarly, the second inserted element to $\NEW_0$ (Rule 1) is moved to $\OLDER_0$ (Rule 2). (b) During the third insertion, both $\OLDEST_0$ and $\OLDER_0$ are full and $\Gamma.$\omerge$_1$\ is called (Rule 3) to move elements from $\OLDEST_0\cup \OLDER_0$ to $\NEW_1$. The third insertion is moved to $\OLD_0$ (Rules 1 and 2).}    \label{fig:sddFig}
\end{figure}
...
\smallskip\noindent\textbf{Phases of \textup{\omerge} framework.}\label{sec:oblivious_merge}
