\chapter{FLAQR : A programming model to apply consensus and replication securely} \label{ch:flaqr}
...
\begin{figure*}
  ...
  \centering
  \includegraphics[scale=0.28]{chapters/pictures/tradeoff1.pdf}
...
\begin{definition}[Initial blame constraint]
  \label{bist}
  For toleration set $\trans{\quo}$ of the form
  $\{\reachn{(p^1_1 \wedge ... \wedge p^1_{m_1})}, ..., \reachn{(p^{k}_1 \wedge ... \wedge p^{k}_{m_{k}})} \}$ 
  the initial blame constraint $\blame_{init}$ is defined as a (logical) disjunction
  of conjunctions:
  {\small
  \begin{align*}
